        ...
        <table>
            ...
            <?php foreach ($result['aspects'] as $aspect): ?>
                <tr class="<?= $aspect->isDominant ? 'row--dominant' : '' ?>">
                    <td class="text-center"><span class="astro-glyph"><?= \Astro\Glyph\SymbolGlyph::getPlanetGlyphByName($aspect->planet1Name) ?></span></td>
                    <td class="text-center"><span class="astro-glyph"><?= \Astro\Glyph\SymbolGlyph::getAspectGlyph($aspect->aspectDegrees) ?></span></td>
                    <td class="text-center"><span class="astro-glyph"><?= \Astro\Glyph\SymbolGlyph::getPlanetGlyphByName($aspect->planet2Name) ?></span></td>
                    <td class="<?= $aspect->isOutOfSign ? 'orb--out-of-sign' : '' ?>"><?= number_format($aspect->orb, 2) ?>°<?= $aspect->isOutOfSign ? ' B' : '' ?></td>
                    <td class="text-center"><?= \Astro\Helpers\Formatter::formatLongitudeWithGlyph($aspect->planet1Longitude) ?></td>
                    <td class="text-center"><?= \Astro\Helpers\Formatter::formatLongitudeWithGlyph($aspect->planet2Longitude) ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
